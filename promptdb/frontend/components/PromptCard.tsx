...
const PromptCard = ({text, sampleInput, sampleOutput, description, id}: {text: string, sampleInput: string, sampleOutput: string, description: string, id: string}) => {
    return (
        <Link href={`/prompt/gpt3/${id}`}>
            <div className="card" style={{width: '18rem', margin: '10px 10px 10px 10px'}}>
